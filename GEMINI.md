...
1.  **로그인 처리:**
    *   로그인 성공 후 백엔드로부터 응답 본문에 포함된 **액세스 토큰**을 수신합니다.
    *   수신된 액세스 토큰을 **메모리(in-memory)**에 저장합니다. (`localStorage`나 `sessionStorage` 사용은 지양하며, 현재 구현된 `lib/auth-storage.ts`는 개발 편의를 위한 것이므로 프로덕션에서는 메모리 저장 로직으로 변경해야 합니다.)
    *   백엔드에서 설정한 HTTP-only 리프레시 토큰 쿠키에는 직접 접근하거나 조작하지 않습니다.

2.  **API 요청 시:**
    *   보호된 API 엔드포인트로 요청을 보낼 때마다 메모리에 저장된 액세스 토큰을 `Authorization` 헤더에 `Bearer <AccessToken>` 형식으로 포함하여 전송합니다.
    *   (권장) `axios` 인터셉터와 같은 기능을 사용하여 모든 API 요청에 자동으로 토큰을 삽입하는 로직을 구현하면 편리합니다.

3.  **액세스 토큰 만료 처리:**
    *   API 요청이 401 Unauthorized 응답을 받으면, 액세스 토큰이 만료되었을 가능성이 높습니다.
    *   이 경우, **리프레시 토큰 엔드포인트**로 새로운 액세스 토큰을 요청하는 함수를 호출합니다.
    *   리프레시 토큰 요청이 성공하면, 새로 발급받은 액세스 토큰으로 메모리 저장소를 업데이트하고, 이전에 실패했던 원래 API 요청을 자동으로 재시도합니다.
    *   리프레시 토큰 요청도 실패하면 (예: 리프레시 토큰도 만료되거나 유효하지 않음), 사용자에게 재로그인이 필요함을 알리고 로그인 페이지로 리디렉션합니다.

4.  **로그아웃:**
    *   메모리에 저장된 액세스 토큰을 삭제합니다.
    *   (선택 사항) 백엔드의 로그아웃 엔드포인트를 호출하여 서버 측에서 리프레시 토큰을 무효화하도록 요청합니다. (이렇게 하면 리프레시 토큰 쿠키가 만료될 때까지 기다릴 필요 없이 즉시 로그아웃 효과를 낼 수 있습니다.)
