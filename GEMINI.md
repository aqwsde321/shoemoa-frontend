...
1.  **로그인 처리 및 인증 상태 관리:**
    *   로그인 성공 후 백엔드로부터 응답 본문에 포함된 **액세스 토큰**과 **사용자 역할(role)**을 수신합니다.
    *   수신된 액세스 토큰과 사용자 역할은 **개발 편의를 위해 현재 \`lib/auth-storage.ts\`를 통해 \`localStorage\`에 저장**됩니다.
    *   **프로덕션 환경에서는 XSS 공격 노출 위험을 최소화하기 위해 액세스 토큰을 메모리(in-memory)에 저장하는 로직으로 변경해야 합니다.** (새로고침 시 토큰 유지 등 추가적인 고려 필요)
    *   **중앙 인증 컨텍스트 \`AuthContext\`와 \`useAuth\` 훅이 \`lib/hooks/use-auth.tsx\`에 구현되어 애플리케이션 전반의 인증 상태(로그인 여부, 사용자 역할, 로딩 상태)를 관리합니다.**
...
4.  **액세스 토큰 만료 처리:**
    ...
    *   이 경우, 백엔드의 **리프레시 토큰 엔드포인트**로 새로운 액세스 토큰을 요청하는 함수를 호출합니다. (현재는 Mock 데이터 폴백 로직만 구현되어 있으며, 실제 리프레시 토큰 로직은 백엔드 구현에 따라 프론트엔드에 추가되어야 합니다.)
